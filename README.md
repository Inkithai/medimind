...
| `retrieval.py` | Chunks timeline into Medication / Lab / ClinicalNote / Allergy texts, embeds (OpenAI `text-embedding-3-small` if set else local ONNX MiniLM), indexes via `vector_store` abstraction, single-shot Q&A with a richer response contract (`cross_document`, `low_confidence`, `consult_reason`, sources enriched with `document_type` + `document_url`) and a deterministic guard that forces `recommend_professional_consult=true` on risk/allergy/dosage questions |
