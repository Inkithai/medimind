...
test("counts every active safety category used by dashboard and safety page", () => {
  const report: CrossCheckReport = {
    ...
    concurrent_exposure: [{
      ingredient: "paracetamol", status: "concurrent", window_start: null, window_end: null,
      overlap_days: 1, sources: [], cumulative_daily_dose: null, dosage_unit: null, note: "overlap",
    }],
...
test("does not mislabel medication changes as safety alerts", () => {
  const report: CrossCheckReport = {
    ...
    medication_changes: [{
      medication: "A",
      previous: { date: null, source_file: null, dosage: "5mg", dosage_value: 5, dosage_unit: "mg", frequency: null, frequency_per_day: null, is_as_needed: false },
      current: { date: null, source_file: null, dosage: "10mg", dosage_value: 10, dosage_unit: "mg", frequency: null, frequency_per_day: null, is_as_needed: false },
      sources: [], explanation: "changed", confidence: 0.9,
    }],
  };
...
test("excludes historical non-concurrent interactions", () => {
  const report: CrossCheckReport = {
    ...empty,
    potential_drug_interactions: [{
      medications_involved: ["A", "B"], explanation: "history", severity: "high", confidence: 0.9,
      timing: { status: "not_concurrent", window_start: null, window_end: null, overlap_days: 0, gap_days: 100, note: "apart" },
    }],
  };
  assert.equal(collectSafetyAlerts(report).length, 0);
});
...
test("prefers cited guidance over a duplicate model finding for the same pair", () => {
  const report: CrossCheckReport = {
    ...empty,
    guideline_flagged_combinations: [{ opioid: "Oxycodone", depressant: "Diazepam", plain: "warning" }],
    potential_drug_interactions: [{
      medications_involved: ["Diazepam", "Oxycodone"], explanation: "model", severity: "high", confidence: 0.6,
    }],
  };
