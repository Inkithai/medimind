      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {result.location.resolved_area && (
              <StatusBadge tone="neutral">
                {t("care.nearArea", { area: result.location.resolved_area })}
              </StatusBadge>
            )}
            <StatusBadge tone="brand">
              {t("care.preferenceValue", { preference: availabilityLabel })}
            </StatusBadge>
          </div>
          {result.referral_reason && (
            <div className="rounded-lg bg-brand-50 px-3 py-2">
              <p className="text-xs font-semibold uppercase tracking-wide text-brand-700">
                {t("care.referralReasonTitle")}
              </p>
      ...
      {result.providers.length === 0 ? (
        <Card>
          <CardBody>
            <EmptyState
              ...
              description={result.no_results_message || t("care.noLiveResultsBody")}
            />
          </CardBody>
        </Card>
      ) : (
        <section className="space-y-3" aria-labelledby="live-providers-title">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h2 id="live-providers-title" className="section-title">
              {t("care.providersTitle")}
            </h2>
            <p className="text-xs text-slate-600">
              {t("care.resultCountConfirm", { count: formatNumber(result.providers.length) })}
            </p>
          </div>
          {result.providers.map((provider, index) => (
            <ProviderResultCard
              key={provider.source_provider_id || `${provider.name}-${index}`}
              provider={provider}
              index={index}
            />
          ))}
