    ...
    <section role="region" aria-labelledby={titleId} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
          ...
          {(visit.document_url || visit.storage_path) && (
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const url = onOpenOriginal ? await onOpenOriginal() : visit.document_url;
                  if (!url) throw new Error("No original document URL is available.");
      ...
      {tab === "original" ? (
        ...
        <div id={structuredPanelId} role="tabpanel" aria-labelledby={structuredTabId} tabIndex={0} className="space-y-5 p-5">
          ...
          <Section title="Raw text / OCR processing">
            <RawTextProcessingPanel
              visit={visit}
              processing={processingText}
              error={processTextError}
              onRefresh={async () => {
                ...
                try {
                  if (onProcessText) {
                    ...
                    throw new Error("Raw text processing is unavailable in this view.");
                  }
                  onUpdated?.();
                } catch (err) {
                  setProcessTextError(err instanceof Error ? err.message : String(err));
                } finally {
...
function RawTextProcessingPanel({
  ...
  const raw = visit.raw_text_processing;
