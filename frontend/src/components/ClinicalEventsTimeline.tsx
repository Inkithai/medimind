    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Event-specific dates from documented diagnoses, symptoms, procedures, vital signs, and
            imaging.
          </p>
        </div>
      ...
      <ol className="mt-4 space-y-2 border-l-2 border-slate-100 pl-5">
        {rows.map((row) => (
          <li
            key={row.key}
            className="relative rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-3"
          >
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[11px] text-slate-400">
                  Source: {row.sourceFile || "unknown document"}
                </p>
              </div>
      ...
      <p className="mt-4 rounded-lg bg-slate-50 px-3 py-2 text-xs leading-relaxed text-slate-500">
        These are facts documented in uploaded records, not diagnoses or interpretations generated
        by MediMind. Confirm clinical history with a qualified professional.
      </p>
    </section>
  );
}
