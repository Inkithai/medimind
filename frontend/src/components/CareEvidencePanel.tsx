...
export function CareEvidencePanel({ flag }: { flag: ClinicalFlag }) {
  ...
  return (
    <Card>
      <CardHeader title={t("care.whySuggested")} description={t("care.recordInterpretation")} />
      <CardBody className="space-y-5">
        <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <StatusBadge tone={flag.trigger === "high_risk" ? "danger" : "warning"}>
              {flag.trigger === "high_risk"
                ...
                : t("care.verificationRecommended")}
            </StatusBadge>
          ...
          {flag.evidence && (
            <p className="mt-2 text-sm leading-relaxed text-slate-600">{flag.evidence}</p>
          )}
          {flag.source && (
            <p className="mt-2 text-xs text-slate-500">
              {t("care.flagSource", { source: flag.source })}
            </p>
          )}
        </div>

        <section aria-labelledby="care-evidence-title">
          <h3 id="care-evidence-title" className="text-sm font-semibold text-slate-800">
            {t("care.evidenceTitle")}
          </h3>
          {evidence.length === 0 ? (
            ...
            <ul className="mt-3 space-y-2">
              {evidence.map((item, index) => (
                <li
                  key={`${item.kind}-${item.label}-${item.source_file || ""}-${index}`}
                  className="rounded-lg border border-slate-200 bg-white p-3"
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">
                        {t(EVIDENCE_KEYS[item.kind])}
                      </p>
        ...
        <section
          ...
          aria-labelledby="suggested-reviewer-title"
        >
          <p
            id="suggested-reviewer-title"
            className="text-xs font-semibold uppercase tracking-wide text-brand-700"
          >
            {t("care.suggestedReviewer")}
          </p>
          ...
          {flag.specialty.reason &&
            flag.care_route_explanation &&
            flag.specialty.reason !== flag.care_route_explanation && (
              <p className="mt-2 text-xs leading-relaxed text-slate-600">
                {t("care.whyRoute", { reason: flag.specialty.reason })}
              </p>
            )}
        </section>
        ...
        {alternative && (
          <section
            ...
            aria-labelledby="broader-alternative-title"
          >
            <p
              id="broader-alternative-title"
              className="text-xs font-semibold uppercase tracking-wide text-slate-600"
            >
              {t("care.broaderAlternative")}
            </p>
            <p className="mt-1 text-sm font-semibold text-slate-800">{alternative.label}</p>
            <p className="mt-1 text-sm text-slate-600">{t("care.broaderAlternativeBody")}</p>
          </section>
        )}
