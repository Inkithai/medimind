    ...
    <Card>
      <CardBody className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              ...
              {provider.open_now === true && <StatusBadge tone="success">{t("care.reportedOpen")}</StatusBadge>}
            </div>
          </div>
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600" title={t("care.directoryScoreTitle")}>
            {t("care.directoryMatch", { score: formatNumber(provider.ranking.score, { maximumFractionDigits: 0 }) })}
          </span>
        </div>
        ...
        <dl className="grid gap-x-4 gap-y-2 text-sm sm:grid-cols-2">
          {provider.address && (
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">{t("care.address")}</dt>
              <dd className="mt-0.5 text-slate-700">{provider.address}</dd>
            </div>
          )}
          {distanceKm !== null && (
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">{t("care.distance")}</dt>
              <dd className="mt-0.5 text-slate-700">{t("care.distanceApprox", { distance: formatNumber(distanceKm, { maximumFractionDigits: 1 }) })}</dd>
            </div>
          )}
          {rating !== null && (
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">{t("care.rating")}</dt>
              <dd className="mt-0.5 text-slate-700">
                {t("care.ratingValue", { rating: formatNumber(rating, { minimumFractionDigits: 1, maximumFractionDigits: 1 }) })}
                {typeof provider.rating_count === "number" ? ` (${t("care.ratingCount", { count: formatNumber(provider.rating_count) })})` : ""}
              </dd>
            </div>
          )}
          {provider.phone && (
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">{t("care.contact")}</dt>
              <dd className="mt-0.5 text-slate-700">
        ...
        {hours.length > 0 && (
          <details className="rounded-lg bg-slate-50 px-3 py-2">
            <summary className="cursor-pointer text-sm font-medium text-slate-700">{t("care.directoryHours")}</summary>
        ...
        <details className="rounded-lg border border-slate-100 px-3 py-2">
          <summary className="cursor-pointer text-xs font-medium text-slate-500">{t("care.whyDirectoryMatch")}</summary>
