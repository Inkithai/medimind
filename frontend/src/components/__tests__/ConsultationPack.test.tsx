...
const completeMarkup = renderToStaticMarkup(
  <I18nProvider>
    <ConsultationPack pack={completePack} />
  </I18nProvider>,
);
assert(
  completeMarkup.includes("Prepare for your consultation"),
  "renders consultation-pack heading",
);
...
assert(
  completeMarkup.includes("Dosage or frequency information requires verification"),
  "renders low-confidence item",
);
assert(completeMarkup.includes("46%"), "renders existing low-confidence value");
assert(
  completeMarkup.includes("Can you confirm the correct dosage and frequency?"),
  "renders clinician question",
);
  ...
  documents_to_bring: [
    {
      ...
      reason: "Contains information relevant to this concern.",
    },
  ],
  medication_records_to_discuss: [],
  allergies: [],
...
const sparseMarkup = renderToStaticMarkup(
  <I18nProvider>
    <ConsultationPack pack={sparsePack} />
  </I18nProvider>,
);
assert(
  !sparseMarkup.includes("View document"),
  "does not create a source link without a valid URL",
);
...
assert(
  ...
  "keeps filename when URL is unavailable",
);
