...
assertIncludes(viewer, "aria-label=\"Highlighted evidence region\"", "normalized evidence renders a visual overlay");
...
const unavailable = renderToStaticMarkup(
  <EvidenceViewer visit={{ ...visit, document_url: undefined }} evidence={{ ...evidence, bbox: null }} />
);
assertIncludes(unavailable, "original file is unavailable", "missing originals have an honest fallback");
assertIncludes(unavailable, "HbA1c 6.1 %", "quote remains visible when the original is unavailable");

const documentMarkup = renderToStaticMarkup(
  <I18nProvider><DocumentViewer visit={visit} /></I18nProvider>
);
assertIncludes(documentMarkup, "Page 2 · View evidence", "structured facts expose evidence controls");
