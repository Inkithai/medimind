...
def _latest_medication_record(timeline: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    candidates = [visit for visit in timeline.get("visits", []) if visit.get("medications") and visit.get("date")]
    if not candidates:
...
def _safety_priorities(timeline: Dict[str, Any], cross_check: Dict[str, Any]) -> List[Dict[str, Any]]:
    priorities = []
    for index, item in enumerate(cross_check.get("allergy_conflicts", []) or []):
        ...
        priorities.append(_priority(
            f"allergy-{index}", "important", "Medication safety",
            f"Review {med} against the documented {allergy} allergy",
            f"My records flag {med} against my {allergy} allergy. Is it appropriate and safe for me?",
            item.get("explanation") or "A medication–allergy conflict was flagged in the record review.",
    ...
    for index, item in enumerate(cross_check.get("potential_drug_interactions", []) or []):
        ...
        priorities.append(_priority(
            f"interaction-{index}", level, "Medication safety",
    ...
    for index, item in enumerate(cross_check.get("conflicting_dosage_instructions", []) or []):
        ...
        priorities.append(_priority(
            f"dosage-{index}", "important", "Medication safety",
            f"Clarify the instructions for {med}",
            f"My records contain different instructions for {med}. Which instruction should I follow?",
            item.get("explanation") or "Different dosage or frequency instructions were extracted across records.",
            evidence,
        ))
    ...
    for index, item in enumerate(cross_check.get("duplicate_prescriptions", []) or []):
        ...
        priorities.append(_priority(
            f"duplicate-{index}", "review", "Medication safety",
            f"Reconcile repeated prescriptions for {med}",
            f"{med} appears more than once in my records. Are these duplicates or intended renewals?",
            item.get("explanation") or "The medicine appeared in multiple prescription entries.",
            evidence,
        ))
    return priorities
...
def _trend_priorities(lab_trends: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for index, trend in enumerate(lab_trends.get("trends", []) or []):
        ...
        if crossing:
            ...
            question = f"{name} moved outside its reported reference range. What might explain this, and does it need follow-up?"
            level = "important"
        elif is_abnormal:
            title = f"Review the {latest_flag} {name} result"
            question = f"My latest {name} result is marked {latest_flag} and the overall direction is {direction}. What does this mean for me?"
            level = "review"
        else:
            title = f"Ask whether {name} needs monitoring"
            question = f"{name} is still in range but trending toward a boundary. When, if at all, should it be checked again?"
            level = "review"
        evidence = [_source(point.get("date"), point.get("source_file")) for point in points]
        priorities.append(_priority(
            f"trend-{index}", level, "Lab trend", title, question,
            trend.get("explanation") or f"The structured trend direction is {direction}.", evidence,
        ))
    return priorities
...
def _change_priorities(changes: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for index, change in enumerate(latest.get("changes", []) or []):
        ...
        priorities.append(_priority(
            f"change-{index}", "review", "Recent change", title,
            f"My two most recent records show: {title}. Can we confirm what this means for my care?",
            change.get("description") or "A structured field changed between the two most recent dated records.",
...
def build_appointment_prep(
    ...
    priorities.sort(key=lambda item: (level_order.get(item["level"], 9), item["category"], item["title"]))

    if not priorities:
        latest_sources = [_visit_source(visit) for visit in dated_visits[-2:]]
        priorities.append(_priority(
            "record-review", "routine", "Record review",
            "Confirm that the medical record is complete",
            "Could we review whether my medication list, allergies, and recent results are complete and up to date?",
            "No specific safety conflict or out-of-range longitudinal trend was available to prioritize.",
            latest_sources,
        ))
    ...
    providers = sorted({
        ...
        for visit in visits if visit.get("provider_or_doctor")
    })
    ...
    return {
        "handoff": {
            ...
            "record_period": {"from": dates[0] if dates else None, "to": dates[-1] if dates else None},
            "providers_documented": providers,
            "known_allergies": timeline.get("known_allergies", []) or [],
            "latest_medication_record": _visit_source(latest_med_record) if latest_med_record else None,
            "latest_documented_medications": _latest_medications(latest_med_record),
            "key_findings": key_findings,
        },
        "priorities": priorities[:10],
        "checklist": [
            {"id": "medications", "text": "Bring medication packaging or a verified list, including supplements."},
            {"id": "symptoms", "text": "Write down symptoms, when they started, and what makes them better or worse."},
            {"id": "questions", "text": "Choose the three questions you most want answered."},
            {"id": "follow-up", "text": "Before leaving, confirm next steps, warning signs, and whether follow-up tests are needed."},
        ],
        "method": "Prepared deterministically from structured records, safety checks, lab trends, and record changes.",
        "note": (
            "This packet helps organize a clinician conversation. It is not a diagnosis or treatment plan. "
            "The medication section is from the latest medication-containing record and may not represent everything currently taken."
        ),
    }
