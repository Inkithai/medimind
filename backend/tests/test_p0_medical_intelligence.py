...
def test_low_platelets_plus_warfarin_is_high():
    tl = _timeline(
        meds=[_med("Warfarin", ["warfarin"])], labs=[_lab("Platelets", "85", "10^9/L", "low")]
    )
...
def test_low_hemoglobin_plus_nsaid():
    tl = _timeline(
        meds=[_med("Brufen", ["ibuprofen"])], labs=[_lab("Hemoglobin", "8.5", "g/dL", "low")]
    )
    f = dl.check_drug_lab_findings(tl)
...
def test_lithium_plus_low_sodium():
    tl = _timeline(
        meds=[_med("Lithium", ["lithium"])], labs=[_lab("Sodium", "128", "mmol/L", "low")]
    )
    f = dl.check_drug_lab_findings(tl)
...
def test_digoxin_plus_low_magnesium():
    tl = _timeline(
        meds=[_med("Digoxin", ["digoxin"])], labs=[_lab("Magnesium", "0.6", "mmol/L", "low")]
    )
    f = dl.check_drug_lab_findings(tl)
...
def test_nitrofurantoin_plus_low_egfr():
    tl = _timeline(
        meds=[_med("Macrobid", ["nitrofurantoin"])], labs=[_lab("eGFR", "28", "mL/min", "low")]
    )
...
def test_fluconazole_plus_high_alt_hepatic():
    tl = _timeline(
        meds=[_med("Diflucan", ["fluconazole"])], labs=[_lab("ALT", "130", "U/L", "high")]
    )
    f = rh.check_renal_hepatic_findings(tl)
...
def test_gout_plus_thiazide():
    f = cc.check_condition_contraindications(
        _timeline(meds=[_med("Hydrot", ["hydrochlorothiazide"])], diagnoses=["Gout"])
    )
...
def test_epilepsy_plus_bupropion():
    f = cc.check_condition_contraindications(
        _timeline(meds=[_med("Wellbutrin", ["bupropion"])], diagnoses=["Epilepsy"])
    )
...
def test_heart_failure_plus_pioglitazone_is_high():
    f = cc.check_condition_contraindications(
        _timeline(meds=[_med("Actos", ["pioglitazone"])], diagnoses=["Heart failure"])
    )
...
def test_dementia_plus_anticholinergic():
    f = cc.check_condition_contraindications(
        _timeline(meds=[_med("Ditropan", ["oxybutynin"])], diagnoses=["Dementia"])
    )
...
def test_reconciliation_classifies_active_discontinued_duplicate():
    tl = _timeline(
        meds=[
            ...
            _med(
                "Metformin", ["metformin"], date="2024-06-01", dosage="1000 mg", duration="90 days"
            ),  # duplicate, dose conflict
            _med(
                "Old Drug", ["terfenadine"], date="2019-01-01", duration="30 days"
            ),  # discontinued
        ]
    )
...
def test_finding_history_diff_and_change_log():
    ...
    fh.snapshot_findings("u", _report([f1, f2]))  # f1 recurred (side effect recorded)
