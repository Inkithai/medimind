...
def _visit(
    date, file, *, patient="John A Doe", meds=None, labs=None, allergies=None, confidence=0.9
):
    return {
        "date": date,
...
def test_detects_complete_same_date_medication_instruction_conflict():
    timeline = {
        "visits": [
            _visit(
                "2025-01-10", "one.pdf", meds=[_med("Brand A", "5 mg", ingredients=["Amlodipine"])]
            ),
            _visit(
                "2025-01-10", "two.pdf", meds=[_med("Brand B", "10 mg", ingredients=["amlodipine"])]
            ),
        ]
    }
    report = check_record_integrity(timeline)
...
def test_detects_no_known_allergies_against_named_allergy():
    ...
    issue = next(
        issue
        for issue in check_record_integrity(timeline)["issues"]
        if issue["category"] == "allergy"
    )
    assert issue["severity"] == "important"
