...
def test_delete_workspace_waits_for_an_active_upload():
    with mock.patch.object(api.jobs, "list_jobs", return_value=[{"status": "processing"}]), \
...
def test_delete_workspace_removes_originals_durable_data_and_memory_caches():
    ...
    with mock.patch.object(api.jobs, "list_jobs", return_value=[]), \
         ...
         mock.patch.object(api.db, "delete_workspace_data", return_value={"documents": 2}) as delete_data, \
