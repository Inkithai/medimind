...
def _flag(flag_id, issue_type):
    return {"id": flag_id, "issue_type": issue_type, "title": "Test flag", "specialty": {"label": "General Physician"}}
...
def test_interaction_evidence_resolves_actual_medications_and_documents():
    ...
    assert {item["source_file"] for item in medication_items} == {"Prescription_01.pdf", "Prescription_02.pdf"}
    assert all(item["document_url"].startswith("https://files.example.test/") for item in medication_items)
    assert any(item["kind"] == "cross_check" and item["label"] == "Potential interaction detected" for item in evidence)


def test_low_confidence_medication_keeps_existing_source_document():
    evidence = build_care_pathway_evidence(
        _flag("medication-confidence-2", "low_confidence_medication"), TIMELINE, CROSS_CHECK, LAB_TRENDS
    )
...
def test_allergy_conflict_connects_actual_medication_and_recorded_allergy():
    ...
    assert any(item["kind"] == "medication" and item["label"] == "Medication A" for item in evidence)
...
def test_dosage_conflict_points_to_existing_prescription_record():
    evidence = build_care_pathway_evidence(
        _flag("conflicting_dosage_instructions-confidence-0", "low_confidence_dosage"), TIMELINE, CROSS_CHECK, LAB_TRENDS
    )
    assert any(item["kind"] == "medication" and item["source_file"] == "Prescription_01.pdf" for item in evidence)
...
def test_unknown_flag_returns_empty_evidence_without_inference():
    assert build_care_pathway_evidence(_flag("unknown-0", "unknown_flag"), TIMELINE, CROSS_CHECK, LAB_TRENDS) == []
...
def test_enrichment_keeps_existing_fields_and_adds_route_evidence():
    flag = {
        ...
        "confidence": 0.91,
        "specialty": {"id": "pharmacy", "label": "Pharmacist / prescribing doctor", "provider_query": "pharmacy"},
    }
