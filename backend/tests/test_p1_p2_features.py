...
def test_vital_trends_normal_pulse():
    tl = _timeline(
        vitals=[{"name": "Pulse", "value": "72", "unit": "bpm", "measured_at": "2024-01-01"}]
    )
    out = vt.track_vital_trends(tl)
...
def test_symptom_dry_cough_finds_ace_inhibitor():
    tl = _timeline(
        meds=[{"name": "Lisinopril", "ingredients": ["lisinopril"], "date": "2024-01-01"}]
    )
...
def test_alert_management_suppresses_overridden_and_dedups():
        ...
        "potential_drug_interactions": [
            {
                "finding_kind": "ddi",
                "rule": "warfarin + nsaid",
                "medications_involved": ["Warfarin", "Brufen"],
                "severity": "high",
            },
            {
                "finding_kind": "ddi",
    ...
    cf.record_feedback(
        "u", fkey, "overridden", reason="reviewed", finding_kind="ddi", rule="ace + k"
    )
...
def test_patient_data_augments_timeline():
    pd.reset("u")
    pd.record_measurement(
        "u", "Blood Pressure", "140/90", unit="mmHg", measured_at="2024-05-01", kind="vital"
    )
    base = _timeline()
...
def test_secure_messaging_thread():
    ...
    sm.send_message(
        "u", "Follow-up: is the dose ok?", provider="Dr Smith", thread_id=m1["thread_id"]
    )
