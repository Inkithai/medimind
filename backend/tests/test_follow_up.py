...
def _timeline():
    visits = [
        _visit("2025-01-01", "one.pdf", meds=[{"name": "A", "ingredients": ["A"], "dosage": "5 mg", "frequency": "daily"}]),
        _visit("2025-02-01", "two.pdf", meds=[{"name": "A", "ingredients": ["A"], "dosage": "10 mg", "frequency": "daily"}]),
    ]
    return {"visits": visits, "known_allergies": [], "medications_timeline": [], "lab_results_timeline": []}
...
def test_builds_stable_high_priority_medication_task():
    ...
    assert task["id"] == next(task for task in second["tasks"] if task["title"] == task["title"])["id"]
...
def test_includes_record_integrity_verification_with_both_sources():
    ...
    ], "known_allergies": [], "medications_timeline": [], "lab_results_timeline": []}
...
def test_lab_task_does_not_invent_a_retest_deadline():
    trends = {"trends": [{
        ...
        "data_points": [
            {"date": "2025-01-01", "source_file": "one.pdf", "value": "90", "flag": "normal"},
            {"date": "2025-02-01", "source_file": "two.pdf", "value": "130", "flag": "high"},
        ],
    }]}
...
def test_empty_findings_produce_routine_record_review():
    timeline = {"visits": [_visit("2025-01-01", "one.pdf")], "known_allergies": [], "medications_timeline": [], "lab_results_timeline": []}
    report = build_follow_up_plan(timeline, {}, {"trends": []})
