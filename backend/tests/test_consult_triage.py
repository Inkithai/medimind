...
def test_duplicates_and_dosage_conflicts_route_to_pharmacist():
    cross_check = {
        **EMPTY_CROSS_CHECK,
        "duplicate_prescriptions": [
            {"medication": "Metformin", "explanation": "dup", "confidence": 0.9}
        ],
        "conflicting_dosage_instructions": [
            {"medication": "Metformin", "explanation": "conflict", "confidence": 0.85}
        ],
    }
    report = generate_consult_triage(cross_check)
...
def test_dosage_ceiling_findings_route_to_doctor_urgent():
    dosage = {
        "findings": [
            {
                "kind": "above_max_daily_dose",
                "medication": "Paracetamol",
                "ingredient": "paracetamol",
                "explanation": "5000 mg/day",
                "confidence": 0.95,
            }
        ]
    }
...
def test_referral_items_sorted_most_urgent_first_and_doctor_wins_type():
    cross_check = {
        **EMPTY_CROSS_CHECK,
        ...
        "allergy_conflicts": [
            {"medication": "A", "allergy": "B", "explanation": "a", "confidence": 0.9}
        ],
    }
    lab_trends = {
        "trends": [
    ...
    assert urgencies == sorted(
        urgencies, key={"routine": 0, "soon": 1, "urgent": 2}.get, reverse=True
    )
    assert report["consult_type"] == "doctor"
    assert report["urgency"] == "urgent"
