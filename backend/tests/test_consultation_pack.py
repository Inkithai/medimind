...
def test_high_severity_interaction_lists_relevant_documents_and_medications():
    ...
    assert {med["name"] for med in pack["medication_records_to_discuss"]} == {
        "Medication A",
        "Medication B",
    }
...
def test_low_confidence_medication_has_verification_item_and_safe_question():
    ...
    assert (
        pack["low_confidence_items"][0]["label"] == "Medication information requires verification"
    )
...
def test_low_confidence_dosage_has_correct_question():
    ...
    assert (
        pack["low_confidence_items"][0]["label"]
        == "Dosage or frequency information requires verification"
    )
...
def test_missing_allergy_data_returns_empty_allergy_list():
    timeline = {
        **TIMELINE,
        "known_allergies": [],
        "visits": [{**TIMELINE["visits"][0], "allergies_noted": []}],
    }
    pack = _pack(
        _flag("allergy-0", "allergy_conflict", confidence=0.89), _snapshot(timeline=timeline)
    )
...
def test_duplicate_document_references_are_deduplicated():
    ...
    assert [doc["source_file"] for doc in pack["documents_to_bring"]].count(
        "Prescription_01.pdf"
    ) == 1
