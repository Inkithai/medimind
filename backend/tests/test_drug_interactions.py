...
def test_merge_skips_pairs_the_llm_already_flagged():
    report = {
        "potential_drug_interactions": [
            {"medications_involved": ["warfarin", "ibuprofen"], "explanation": "LLM found it", "severity": "high", "confidence": 0.9},
        ],
    }
    kb = [
        {"medications_involved": ["Warfarin", "Ibuprofen"], "explanation": "KB", "severity": "high", "confidence": 0.97, "source": "curated_knowledge_base", "rule": "x"},
        {"medications_involved": ["Sertraline", "Tramadol"], "explanation": "KB", "severity": "moderate", "confidence": 0.97, "source": "curated_knowledge_base", "rule": "y"},
    ]
...
def test_merge_into_empty_report_creates_key():
    merged = merge_into_report({}, [
        {"medications_involved": ["A", "B"], "explanation": "KB", "severity": "low", "confidence": 0.97, "source": "curated_knowledge_base", "rule": "z"},
    ])
...
def test_cross_check_prescriptions_merges_kb_findings(monkeypatch):
    ...
    monkeypatch.setattr(
        medical_extractor, "_completion_resilient", lambda **kwargs: llm_report
    )
    timeline = {
