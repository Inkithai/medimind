...
def _identity_issues(visits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    named = [
        (visit, _name_parts(visit.get("patient_name")))
        for visit in visits
        if _name_parts(visit.get("patient_name"))
    ]
    ...
    return [
        _issue(
            ...
            "Uploaded records contain patient names that do not match on first and last name. This may indicate a mixed-patient workspace or an extraction error.",  # noqa: E501
            variants,
            "Verify the names against the original documents before relying on combined trends or answers.",  # noqa: E501
...
def _lab_issues(visits: List[Dict[str, Any]], dayfirst: bool = True) -> List[Dict[str, Any]]:
    groups: DefaultDict[Tuple[str, str], List[Tuple[Dict[str, Any], Dict[str, Any]]]] = defaultdict(
        list
    )
    for visit in visits:
    ...
    for (date, _), entries in groups.items():
        if len(entries) < 2:
            continue
        values: DefaultDict[Tuple[str, str], List[Tuple[Dict[str, Any], Dict[str, Any]]]] = (
            defaultdict(list)
        )
        ...
        issues.append(
            _issue(
                "lab",
                "review",
                f"Same-date {test_name} results differ",
                (
                    "The same test and date appear with different values or units. They could be separate samples, a unit conversion, a corrected result, or an extraction error."  # noqa: E501
                    if units_differ
                    else "The same test and date appear with different values. They could be separate samples, a corrected result, or an extraction error."  # noqa: E501
                ),
                variants,
                "Compare the original reports, including collection times and units; ask the laboratory or clinician which result should be used.",  # noqa: E501
...
def _medication_issues(visits: List[Dict[str, Any]], dayfirst: bool = True) -> List[Dict[str, Any]]:
    ...
    for (date, _), entries in groups.items():
        if len(entries) < 2:
            continue
        instructions: DefaultDict[Tuple[str, str], List[Tuple[Dict[str, Any], Dict[str, Any]]]] = (
            defaultdict(list)
        )
        ...
        issues.append(
            _issue(
                "medication",
                "important",
                f"Same-date instructions differ for {name}",
                "Records dated the same day contain different complete dosage/frequency instructions for the same medication or active ingredient.",  # noqa: E501
                variants,
                "Verify the original prescription and ask a clinician or pharmacist which instruction is intended before making any change.",  # noqa: E501
...
def _allergy_issues(visits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    return [
        _issue(
            "allergy",
            "important",
            "Allergy statements conflict across records",
            "At least one record says there are no known allergies while another names a specific allergy.",  # noqa: E501
            [
                {
            ...
            "Confirm the allergy history with the patient and clinician; do not remove an allergy based only on a conflicting document.",  # noqa: E501
...
def check_record_integrity(timeline: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "method": "Deterministic comparison of structured fields; no generative model chooses which source is correct.",  # noqa: E501
        "note": (
            "A discrepancy is a prompt to verify the original records, not proof that either source is wrong. "  # noqa: E501
            "Different collection times, corrected reports, unit conversions, and extraction uncertainty can explain apparent conflicts."  # noqa: E501
        ),
    }
