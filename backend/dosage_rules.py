...
DOSAGE_RULES: Dict[str, Dict[str, Any]] = {
    ...
    "metformin": {
        ...
        "notes": "Common ceiling 2550 mg/day (850 mg three times daily); some references allow 3000 mg/day extended-release.",  # noqa: E501
    },
    ...
    "omeprazole": {
        ...
        "notes": "Routine ceiling 40 mg/day; higher divided doses only in hypersecretory conditions.",  # noqa: E501
    },
    ...
    "warfarin": {
        ...
        "max_frequency_per_day": 1,
        "notes": "Highly individualized by INR; doses above 10 mg/day are unusual and worth confirming.",  # noqa: E501
    },
    "levothyroxine": {
...
def _dose_to_mg(
    ...
    if unit_lower in ("iu", "international unit", "international units"):
        ...
        if factor is not None:
            return value * float(factor), (f"converted exactly at {float(factor):g} mg per IU")
    return None, None


def check_dosages(timeline: Dict[str, Any], reference_date: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    for med in scoped.get("medications_timeline", []) or []:
        ...
        ingredients = [
            str(i).strip().lower() for i in (med.get("ingredients") or []) if str(i).strip()
        ]
            ...
            skipped.append(
                {
                    **source,
                    "medication": display,
                    "reason": "combination product — per-ingredient dose cannot be attributed deterministically",  # noqa: E501
                }
            )
            continue
        ...
        if rule is None:
            skipped.append(
                {**source, "medication": display, "reason": f"no dosage rule for '{ingredient}'"}
            )
            continue
        ...
        if not isinstance(dosage_value, (int, float)) or not dosage_unit:
            skipped.append(
                {**source, "medication": display, "reason": "dose not normalized to value+unit"}
            )
            continue
        ...
        def _finding(kind: str, explanation: str) -> Dict[str, Any]:
            ...
            return {
                ...
                **source,
                ...
                "confidence": (
                    DOSAGE_CHECK_CONFIDENCE if not unit_assumption else ASSUMED_STRENGTH_CONFIDENCE
                ),
                "source": "dosage_rules",
            }
