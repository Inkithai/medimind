...
def _score_specialty(provider: Dict[str, Any], specialty: Dict[str, Any]) -> Tuple[float, str]:
    ...
    if matched:
        return 1.0, f"Source metadata matches the selected provider category ({', '.join(sorted(set(matched))[:3])})."
    ...
    if any(marker in provider_text for marker in broad_markers):
        return 0.55, "Listed as a broad healthcare provider, but the source did not provide a matching specialty tag."
...
def _score_distance(distance_km: Optional[float]) -> Tuple[Optional[float], str]:
    if distance_km is None:
        return None, "Distance was unavailable because the source did not provide usable coordinates."
    # Smooth 0–1 decay over 30 km; distance itself remains visible to the user.
    return max(0.0, 1.0 - min(float(distance_km), 30.0) / 30.0), f"Approximately {distance_km:.1f} km from the searched area."
...
def _availability_match(provider: Dict[str, Any], preference: str) -> Tuple[Optional[float], str]:
    ...
    if not isinstance(hours, list) or not hours:
        return None, "Opening-hours information was not provided by the live directory, so availability was not used for ranking."
    ...
    if len(hours) == 1 and ":" not in str(hours[0]):
        return None, "Opening-hours text was provided but was not in a safely interpretable format; availability was not used for ranking."
    ...
    if preference == "today":
        ...
        if today_lines and not any("closed" in line for line in today_lines):
            return 0.75, "Regular hours are listed for today; live appointment availability is not guaranteed."
    ...
    if preference == "this_week":
        if any("closed" not in line for line in normalized_hours):
            return 0.75, "Regular hours are listed this week; live appointment availability is not guaranteed."
        return 0.0, "No regular opening hours this week were provided."

    if preference == "weekends":
        weekend_lines = [line for line in normalized_hours if line.startswith("saturday") or line.startswith("sunday")]
        if any("closed" not in line for line in weekend_lines):
            return 1.0, "Regular weekend hours are listed; live appointment availability is not guaranteed."
    ...
    if preference == "evenings":
        ...
        if any(hour >= 17 for hour in closing_hours):
            return 1.0, "Regular hours indicate an evening closing time; live appointment availability is not guaranteed."
        if closing_hours:
            return 0.0, "Listed regular hours do not show an evening closing time."
        return None, "Opening-hours text could not be safely interpreted for evenings, so availability was not used for ranking."
...
def ranking_method_description() -> str:
    return (
        "Directory match scores use only source-returned provider category/specialty metadata, calculated distance from the searched area, "
        "and rating or regular opening-hours information only when those fields are actually returned by the live directory. "
