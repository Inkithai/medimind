...
# gemini: export GEMINI_API_KEY=AIza...  # aistudio.google.com/app/apikey; quota is project/model-specific
...
- `MODEL` default `openai/gpt-oss-120b` (groq) or `gemini-3.6-flash` (gemini). Overridable via `GROQ_MODEL` / `GEMINI_MODEL` / `LLM_MODEL`.
- `VISION_MODEL` default `qwen/qwen3.6-27b` (groq) or `gemini-3.6-flash` multimodal (gemini). Gemini 2.0 Flash was shut down on 2026-06-01 and must not be configured; its retired endpoint can misleadingly return HTTP 429 with quota `limit: 0`. Overridable via `GROQ_VISION_MODEL` / `GEMINI_VISION_MODEL` / `LLM_VISION_MODEL`.
...
2. Temporary rate limits (429) honor either the provider's `Retry-After` header or Gemini's body-level `google.rpc.RetryInfo.retryDelay`, capped by `GEMINI_MAX_RATE_LIMIT_RETRIES` / `GROQ_MAX_RATE_LIMIT_RETRIES` / `LLM_MAX_RATE_LIMIT_RETRIES` (default 5). A daily/account quota, explicit `limit: 0`, or known retired Gemini model raises `ProviderRateLimitError` on the first response; the upload worker circuit-breaker then marks queued files without sending more doomed requests. The OpenAI SDK's own retry loop is disabled (`max_retries=0`).
...
4. If every non-quota rung fails, one last-resort vision repair runs with the original image. There is intentionally no text-only repair for an image because it would fabricate empty clinical output; the caller receives a retryable failure instead.
...
`process_document(file_path, progress_callback=None)`:

- Emits truthful `reading` and `extracting` callbacks per file (including scanned-PDF page counts) without coupling extraction to the job store.
