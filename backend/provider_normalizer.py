...
def _distance(
    payload: ProviderSourcePayload, latitude: Optional[float], longitude: Optional[float]
) -> Optional[float]:
...
def _google_record(
    record: Dict[str, Any], payload: ProviderSourcePayload
    ...
    regular_hours = (
        record.get("regularOpeningHours")
        if isinstance(record.get("regularOpeningHours"), dict)
        else {}
    )
    current_hours = (
        record.get("currentOpeningHours")
        if isinstance(record.get("currentOpeningHours"), dict)
        else {}
    )
    opening_hours = _unique_strings(
        regular_hours.get("weekdayDescriptions", [])
        if isinstance(regular_hours.get("weekdayDescriptions"), list)
        else []
    )
    source_types = _unique_strings(
        [
            record.get("primaryType"),
            *(record.get("types", []) if isinstance(record.get("types"), list) else []),
        ]
    )
    rating = _number(record.get("rating"))
    rating_count_raw = record.get("userRatingCount")
    rating_count = (
        int(rating_count_raw)
    ...
    return {
        ...
        "phone": _text(record.get("nationalPhoneNumber"))
        or _text(record.get("internationalPhoneNumber")),
...
def _osm_address(tags: Dict[str, Any]) -> Optional[str]:
    ...
    parts = _unique_strings(
        [
            " ".join(
                part
                for part in [_text(tags.get("addr:housenumber")), _text(tags.get("addr:street"))]
                if part
            ),
...
def _osm_record(record: Dict[str, Any], payload: ProviderSourcePayload) -> Optional[Dict[str, Any]]:
    ...
    for key in ("healthcare:speciality", "medical_specialty", "speciality"):
        ...
        if raw_specialty:
            flat_specialties.extend(
                part.strip() for part in raw_specialty.replace(";", ",").split(",") if part.strip()
            )
    ...
    return {
        ...
        if _text(tags.get("opening_hours"))
        else [],
        "open_now": None,
        "map_url": map_url,
...
def _geoapify_record(
    ...
    return {
        ...
        if any("clinic" in str(c) for c in categories)
        else ("hospital" if any("hospital" in str(c) for c in categories) else None),
        ...
        or _text(raw.get("phone")),
        "opening_hours": [_text(raw.get("opening_hours"))]
        if _text(raw.get("opening_hours"))
        else [],
        "open_now": None,
        "map_url": map_url,
...
def normalize_provider_records(payload: ProviderSourcePayload) -> List[Dict[str, Any]]:
    ...
    for raw in payload.records:
        ...
        key = (
            candidate.get("source_provider_id")
            or "|".join(
