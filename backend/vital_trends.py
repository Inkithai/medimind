...
_VITALS: Dict[str, Tuple[str, Tuple[str, ...]]] = {
    ...
    "oxygen_saturation": ("Oxygen saturation", ("oxygen saturation", "spo2", "o2 sat", "saturation")),
    "weight": ("Weight", ("weight",)),
    "temperature": ("Temperature", ("temperature", "temp")),
    "respiratory_rate": ("Respiratory rate", ("respiratory rate", "respiration rate", "rr", "breathing rate")),
...
def _series_for(timeline: Dict[str, Any], key: str, aliases: Tuple[str, ...]) -> List[Dict[str, Any]]:
    ...
    for entry in (timeline.get("vital_signs_timeline") or []):
        ...
        if key == "blood_pressure":
            ...
            if bp is None:
                continue
            rows.append({"date": date, "value": entry.get("value"),
                         "systolic": bp[0], "diastolic": bp[1], "unit": unit,
                         "entry": entry})
        else:
            num = _parse_number(entry.get("value"))
            if num is None:
                continue
            rows.append({"date": date, "value": entry.get("value"), "numeric": num,
...
def _latest_signal(key: str, latest: Dict[str, Any]) -> Tuple[Optional[str], str]:
    ...
    if key == "blood_pressure":
        ...
        if s >= sh or d >= dh:
            return "high", (f"Your most recent blood pressure is "
        ...
        if s >= 130 or d >= 80:
            return "borderline", (f"Your most recent blood pressure is {s:g}/{d:g} "
                                  "mmHg, which is in the elevated/borderline range.")
        return None, f"Your most recent blood pressure is {s:g}/{d:g} mmHg, within the normal screening range."
