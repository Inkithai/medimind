...
def _row(user_id: str, kind: str, data: Dict[str, Any], identity: Dict[str, Any]) -> Dict[str, Any]:
    raw_date = data.get("date") or data.get("onset_date") or data.get("procedure_date") or data.get("study_date")
...
def _event_rows(user_id: str, timeline: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for collection, event_type in (
        ...
        for index, item in enumerate(timeline.get(collection) or []):
            ...
            rows.append(_row(user_id, "event", data, {**_source_identity(item, index), "type": event_type}))
    return rows


def _safety_items(cross_check: Dict[str, Any], dosage_report: Dict[str, Any]) -> Iterable[tuple[str, Dict[str, Any]]]:
    for field, kind in (
...
def build_projection(
    ...
    for index, med in enumerate(timeline.get("medications_timeline") or []):
        if not isinstance(med, dict):
            continue
        ingredients = [str(v).strip().lower() for v in med.get("ingredients") or [] if str(v).strip()]
        ...
        if not any(row["id"] == med_id for row in projection["clinical_medications"]):
            projection["clinical_medications"].append({
                "id": med_id, "user_id": user_id, "document_id": None,
                "event_date": None,
                "data": {"name": med.get("name"), "ingredients": med.get("ingredients") or [], "normalized_key": medication_key},
            })
        prescription = {**med, "medication_id": med_id}
        prescription_row = _row(
            user_id, "prescription", prescription, {**_source_identity(med, index), "medication": medication_key}
        )
    ...
    if allergy_entries:
        for index, item in enumerate(allergy_entries):
            ...
            projection["clinical_allergies"].append(_row(
        ...
        for index, allergy in enumerate(timeline.get("known_allergies") or []):
            ...
            projection["clinical_allergies"].append(_row(user_id, "allergy", data, {"allergy": allergy, "index": index}))

    for index, lab in enumerate(timeline.get("lab_results_timeline") or []):
        if isinstance(lab, dict):
            projection["clinical_lab_results"].append(_row(
                user_id, "lab_result", lab, {**_source_identity(lab, index), "test": lab.get("test_name")}
            ))
    ...
    for index, (kind, item) in enumerate(_safety_items(cross_check, dosage_report)):
        subject = (
            item.get("medications_involved") or item.get("medication") or
            item.get("ingredient") or item.get("rule") or item.get("explanation")
        )
        ...
        if isinstance(sources, list):
            sources = sorted(sources, key=lambda value: json.dumps(value, sort_keys=True, default=str))
