...
def generate_care_gaps(
    ...
    med_text = " ".join(
        str(m.get("name") or "") + " " + " ".join(m.get("ingredients") or []) for m in meds
    ).lower()
    ...
    if isinstance(age, int):
        if age >= 50:
            add(
                ...
                "From age 50 (and often earlier in some guidelines), colorectal screening "
                "(e.g. stool test or colonoscopy) is advised. No result found in your records.",
        ...
        if age >= 40:
            add(
                "screening",
                "Cholesterol / cardiovascular risk check",
        ...
        if age >= 60:
            add(
                "screening",
                "Bone-density (osteoporosis) assessment",
        ...
        if 18 <= age <= 50 and "tetanus" not in med_text:
            add(
                "vaccination",
                "Tetanus booster",
                "A tetanus-diphtheria booster is advised every 10 years. Check when yours was last given.",  # noqa: E501
            )
        ...
        if isinstance(age, int):
            ...
            if 50 <= age <= 74:
                add(
                    ...
                    "Breast-cancer screening (mammogram) is generally advised every 2 years from 50–74.",  # noqa: E501
                )
    ...
    if _has_condition("diabetes", conditions):
        add(
            ...
            "eye and foot checks, kidney function, and blood pressure.",
            "soon",
        )
    ...
    if _has_condition("asthma", conditions) or _has_condition("copd", conditions):
        add(
            ...
            "inhaler technique check are current.",
            "routine",
        )
    ...
    if (
        _has_condition("atrial fibrillation", conditions)
        or _has_condition("afib", conditions)
        or "a-fib" in " ".join(conditions)
    ):
        add(
            "monitoring",
            "Stroke-prevention review",
            "With atrial fibrillation on record, check that stroke-risk assessment and "
            "any blood-thinning treatment are current and reviewed.",
            "soon",
        )
    if _has_condition("hypothyroid", conditions) or _has_condition(
    ...
    if (
        _has_condition("hyperlipid", conditions)
        or _has_condition("high cholesterol", conditions)
        or _has_condition("dyslipid", conditions)
    ):
        add(
            "monitoring",
            "Cholesterol monitoring",
            "With a cholesterol disorder on record, check that lipid levels and any "
            "treatment are reviewed periodically.",
            "routine",
        )
    if _has_condition("osteoporosis", conditions) or _has_condition("low bone density", conditions):
        add(
            ...
            "assessment, calcium/vitamin D, and falls-prevention advice are current.",
            "routine",
        )
    if _has_condition("chronic liver", conditions) or _has_condition("cirrhosis", conditions):
        add(
            ...
            "and liver-cancer screening) and liver-function tests are current.",
            "soon",
        )
    if _has_condition("heart failure", conditions):
        add(
            ...
            "monitoring, and kidney function checks are current.",
            "soon",
        )
    ...
    if any(d in med_text for d in ("dabigatran", "rivaroxaban", "apixaban", "edoxaban")):
        add(
            ...
            "checks. Confirm these are current.",
            "soon",
        )
    ...
    if "digoxin" in med_text:
        add(
            ...
            "periodically. Confirm these are current.",
            "soon",
        )
    if "metformin" in med_text:
        add(
            ...
            "term) vitamin B12 are reviewed periodically.",
            "routine",
        )
    if any(
        d in med_text
        for d in ("phenytoin", "carbamazepine", "valproate", "valproic acid", "lamotrigine")
    ):
        add(
            "monitoring",
            "Antiepileptic monitoring",
            "Some antiepileptic medicines need drug-level, liver, or blood-count "
            "monitoring. Confirm these are current.",
            "routine",
        )
    if any(
        d in med_text
    ...
    if isinstance(age, int) and 18 <= age <= 79:
        add(
            ...
            "A one-time hepatitis C test is recommended for most adults. Not seen in your records.",
            "routine",
        )
        add(
            "screening",
            "HIV screening",
            "An HIV test is recommended at least once for adults. Not seen in your records.",
            "routine",
        )
    ...
    if _has_condition("smok", conditions) or _has_condition("tobacco", conditions):
        if isinstance(age, int) and 50 <= age <= 80:
            add(
                "screening",
                "Lung-cancer screening (smoking history)",
        ...
        add(
            ...
            "With smoking on record, ask about cessation support if you still smoke.",
            "soon",
        )
    ...
    return {
        ...
        "count": len(gaps),
        "note": (
            "These are general preventive-care reminders based on widely used adult "
